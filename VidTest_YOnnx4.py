...
# Load ONNX model
session = onnxruntime.InferenceSession("/home/krti/yolov8n.onnx", providers=["CUDAExecutionProvider"])

# Label class YOLOv8 (COCO dataset)
...
while cap.isOpened():
    ...
    predictions = outputs[0][0]  # Output YOLOv8
    
    # Process output
    for pred in predictions:
        x_center, y_center, width, height, conf, cls = pred[:6]
        if conf > 0.5:  # Gunakan threshold confidence
            # **Perbaikan Konversi Koordinat**
            x1 = int((x_center - width / 2) * w / 640)  # Skalakan ke ukuran asli
            ...
            if 0 <= int(cls) < len(model_names):
                ...
                label = f"Unknown Class {int(cls)}: {conf:.2f}"
